...
#import discord
from VNDFHelper import hunt_for_rankval, ranktoint, inttorank, findname, findc_rank, rankval_id, c_rankval_id, namegen
...
class NewRank:
    ...
    def __add__(self, by:int):
        ...
        if newint > self.limit:
            raise self.AuthorityError(f"Exceeded authority. Your promotion limit is {self.limit}, you tried to get to {newint}")
    ...
    def make_name(self) -> str: # yes this pretty much just forwards to the helper function, shush
        return namegen(self.who.nick, findname(self.rankval), self.rankval, self.get_c_rank())
